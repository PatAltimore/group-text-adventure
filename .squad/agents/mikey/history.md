...
- **404 Deep Dive Review (2026-04-01):** Full simulation of deploy pipeline confirmed ALL code artifacts are correct: v4 model, ESM, entry point, dependencies, zip structure (flat, forward-slash paths, 39 prod packages). Node.js test-load detected all 4 functions (negotiate, gameHubConnect, gameHubMessage, gameHubDisconnect). Root cause narrowed to deployment settings: `$newSettings` dict in `deploy.ps1` omits `FUNCTIONS_EXTENSION_VERSION=~4` and `AzureWebJobsStorage`, relying on ARM API merge which can fail due to propagation delay after `az functionapp create`. The `criticalKeys` verification also doesn't check `FUNCTIONS_EXTENSION_VERSION`. Findings written to `.squad/decisions/inbox/mikey-deploy-review.md`.
